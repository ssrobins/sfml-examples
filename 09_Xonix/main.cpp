...
int main()
{
    ...
    t2.loadFromFile("assets/gameover.png");
    t3.loadFromFile("assets/enemy.png");
    ...
    while (window.isOpen())
    {
        ...
        for (int i=0;i<M;i++)
        {
            for (int j=0;j<N;j++)
            {
                ...
                sTile.setPosition(static_cast<float>(j*ts), static_cast<float>(i*ts));
                window.draw(sTile);
            }
        }

        sTile.setTextureRect(IntRect(36,0,ts,ts));
        sTile.setPosition(static_cast<float>(x*ts), static_cast<float>(y*ts));
        window.draw(sTile);

        sEnemy.rotate(10);
        for (int i=0;i<enemyCount;i++)
        {
            sEnemy.setPosition(static_cast<float>(a[i].x), static_cast<float>(a[i].y));
            window.draw(sEnemy);
        }
