...
int main()
{
    ...
    for(int i=1;i<=7;i++)
    {
        t[i].loadFromFile("assets/"+std::to_string(i)+".png");
        t[i].setSmooth(true);
    ...
    while (app.isOpen())
    {
        ...
        if (Keyboard::isKeyPressed(Keyboard::Right)) playerX+=0.1f;
        if (Keyboard::isKeyPressed(Keyboard::Left)) playerX-=0.1f;
        ...
        {
            Line &l = lines[n%N];
            l.project(static_cast<int>(playerX*roadW-x), camH, startPos*segL - (n>=N?N*segL:0));
            x+=dx;
            dx+=l.curve;
            ...
            drawQuad(app, grass, 0, static_cast<int>(p.Y), width, 0, static_cast<int>(l.Y), width);
            drawQuad(app, rumble, static_cast<int>(p.X), static_cast<int>(p.Y), static_cast<int>(p.W*1.2f), static_cast<int>(l.X), static_cast<int>(l.Y), static_cast<int>(l.W*1.2f));
            drawQuad(app, road, static_cast<int>(p.X), static_cast<int>(p.Y), static_cast<int>(p.W), static_cast<int>(l.X), static_cast<int>(l.Y), static_cast<int>(l.W));
        }
