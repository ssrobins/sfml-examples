...
Vector2f toCoord(char a,char b)
{
    int x = int(a) - 97;
    int y = 7-int(b)+49;
    return Vector2f(static_cast<float>(x*size), static_cast<float>(y*size));
}
...
int main()
{
    ...
    ConnectToEngine((char*)"assets/stockfish.exe");
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(e))
        {
            ...
            if (e.type == Event::MouseButtonReleased)
            {
                ...
                {
                    isMove=false;
                    Vector2f p = f[n].getPosition() + Vector2f(static_cast<float>(size/2), static_cast<float>(size/2));
                    newPos = Vector2f( static_cast<float>(size*int(p.x/size)), static_cast<float>(size*int(p.y/size)) );
