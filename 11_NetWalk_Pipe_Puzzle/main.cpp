...
int main()
{
    ...
    t2.loadFromFile("assets/comp.png");
    t3.loadFromFile("assets/server.png");
    t4.loadFromFile("assets/pipes.png");
    t4.setSmooth(true);
    ...
    while (app.isOpen())
    {
        ...
        for(int i=0;i<N;i++)
        {
            for(int j=0;j<N;j++)
            {
                ...
                p.angle+=5;
                if (p.angle>p.orientation*90) p.angle= static_cast<float>(p.orientation*90);

                sPipe.setTextureRect(IntRect(ts*kind,0,ts,ts));
                sPipe.setRotation(p.angle);
                sPipe.setPosition(static_cast<float>(j*ts), static_cast<float>(i*ts));sPipe.move(offset);
                app.draw(sPipe);
                ...
                if (kind==1)
                {
                    ...
                    sComp.setPosition(static_cast<float>(j*ts), static_cast<float>(i*ts));sComp.move(offset);
                    app.draw(sComp);
                }
            }
        }
